...
class SVR_dispatch:
    ...
    def __init__(self, Gmax, batt, costs, flags, sen, outDSS, initParams, outES):
        ...
        self.dvdrR = sen["dvdrR"]
        self.dvdrL = sen["dvdrL"]
    ...
    def PTDF_SLP_OPF(self):
        ...
        x, m, LMP = self.__linprogWR(f, Aeq, beq, A, b, lb.T, ub.T)
    ...
    def __buildSensitivityInequality(self, dxdp, x_0, x_lb, x_ub):
        ...
        b = np.concatenate((x_lb.T + x_0.T - aux_dxdp,
                           x_lb.T - x_0.T + aux_dxdp,
                           x_ub.T + x_0.T - aux_dxdp,
                           x_ub.T - x_0.T + aux_dxdp,
                           x_lb.T - x_0.T + aux_dxdp,
                           x_lb.T + x_0.T - aux_dxdp,
                           x_ub.T - x_0.T + aux_dxdp,
    ...
    def __regAin(self, A1, b, R_0):
        ...
        # x^-
        # for the voltage
        A2_v, b2_v = self.__regSensitivityA(self.dvdrL, self.dvdrR)
        # for the flows
        A2_fl, b2_fl = self.__regSensitivityA(self.dpjkdrL, self.dpjkdrR)

        # contatenate both contributions
        A2 = sparse.vstack((A2_fl, A2_v))
        b2 = np.concatenate((b2_fl, b2_v), axis=0)
    ...
    def __regSensitivityA(self, dxdrL, dxdrR):
        ...
        dxdrR = dxdrR.values
        # define A_min
        A2_min = np.block([[dxdrL, np.zeros(dxdrR.shape)],     # -dv/dp * (-tapn)
                           [-dxdrL, np.zeros(dxdrR.shape)],     # dv/dp * (-tapn)
                           [np.zeros(dxdrL.shape), -dxdrR],     # -dv/dp * (-tapn)
                           [np.zeros(dxdrL.shape), dxdrR]])     # dv/dp * (-tapn)
        A2_min = sparse.kron(sparse.csr_matrix(A2_min), sparse.csr_matrix(np.eye(self.pointsInTime)))

        # define A_max
        A2_max = np.block([[-dxdrL, np.zeros(dxdrR.shape)],     # -dv/dp * (tapp)
                           [dxdrL, np.zeros(dxdrR.shape)],     # dv/dp * (tapp)
                           [np.zeros(dxdrL.shape), dxdrR],     # -dv/dp * (tapp)
                           [np.zeros(dxdrL.shape), -dxdrR]])     # dv/dp * (tapp)
        A2_max = sparse.kron(sparse.csr_matrix(A2_max), sparse.csr_matrix(np.eye(self.pointsInTime)))
        # concatenate
        A2 = sparse.vstack((A2_min, A2_max))
        # concatenate
        # define b
    ...
    def __storageSensitivityA(self, row, dxdp):
        ...
        A_b1_aux = np.block([[dxdp[:, row], -dxdp[:, row]],  # -dx/dp * (-Psd+Psc)
                             [-dxdp[:, row], dxdp[:, row]],  # -dx/dp * (-Psd+Psc)
                             [dxdp[:, row], -dxdp[:, row]],  # -dx/dp * (-Psd+Psc)
                             [-dxdp[:, row], dxdp[:, row]]])  # -dx/dp * (-Psd+Psc)
        A_b1_aux1 = sparse.kron(A_b1_aux, sparse.csr_matrix(np.eye(self.pointsInTime)))
        A_b1 = sparse.hstack((A_b1_aux1,
                             sparse.csr_matrix(np.zeros((A_b1_aux1.get_shape()[0],
                                                        self.numBatteries * (self.pointsInTime + 1)))))) # -dx/dp *(0*E)

        # portion related to upper bound
        A_b2_aux = np.block([[-dxdp[:, row], dxdp[:, row]],  # dx/dp *(-Psd+Psc)
                             [dxdp[:, row], -dxdp[:, row]],  # dx/dp *(-Psd+Psc)
                             [-dxdp[:, row], dxdp[:, row]],  # dx/dp *(-Psd+Psc)
                             [dxdp[:, row], -dxdp[:, row]]])  # dx/dp *(-Psd+Psc)
        A_b2_aux1 = sparse.kron(A_b2_aux, sparse.csr_matrix(np.eye(self.pointsInTime)))
        A_b2 = sparse.hstack((A_b2_aux1,
                             sparse.csr_matrix(np.zeros((A_b2_aux1.get_shape()[0],
                                                        self.numBatteries * (self.pointsInTime + 1))))))  # -dx/dp *(0*E)
    ...
    def __linprogWR(self, f, Aeq, beq, A, b, lb, ub):
